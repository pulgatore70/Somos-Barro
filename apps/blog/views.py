...
def index(request):
    ...
    ultimasnoticias = Noticia.objects.all().reverse()[:3]
    context ={
...
def nosotros(request):
    return render(request, 'xtras/nosotros.html',{})
def eventos(request):
    return render(request, 'eventos/eventos.html',{})
def contactos(request):
    return render(request, 'xtras/contacto.html',{})

def noticias(request):
    lista_noticias = Noticia.objects.all()
    context = {
        "noticias": lista_noticias,
        "MEDIA_ROOT": 'media/noticias/'
    }
